	...
	public class SFGGameController : MonoBehaviour 
	{
        ...
        public void CreateRing()
        {
           ...
           // PlayerPrefs.SetFloat("score", score);
            ...
            if (playerObject.moveSpeed < speedLimit)
                playerObject.moveSpeed += ringObject.speedBoost;
            else
                playerObject.moveSpeed = speedLimit;
        }
        ...
        void  Update()
		{
			...
			else
			{
				...
				else
				{
					...
					if ( Input.GetAxisRaw("Mouse X") != 0 || Input.GetAxisRaw("Mouse Y") != 0 || Input.GetMouseButtonDown(0) || Input.touchCount > 0 )    usingMouse = false;

					// If we press gamepad or keyboard arrows, then mouse controls are turned off
					if ( Input.GetAxisRaw("Left Joystick Horizontal") != 0 || Input.GetAxisRaw("Left Joystick Vertical") != 0 )    usingMouse = false;
                    ...
                    if (playerObject)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                playerObject.turnDirection = Vector2.Lerp(playerObject.turnDirection, new Vector2(Input.GetAxisRaw("Left Joystick Horizontal"), Input.GetAxisRaw("Left Joystick Vertical")), Time.deltaTime * playerObject.turnSpeed * 0.1f);
                            }
                        }
                    }
				}
